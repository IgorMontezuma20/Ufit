...
class ExerciseDetailsViewModel @Inject constructor(
    ...
    fun getImage(queries: Map<String, String>) = viewModelScope.launch {
    ...
    private suspend fun getExercisesSafeCall(queries: Map<String, String>) {
        ...
        if(hasInternetConnection()){
            ...
            Log.d("Retorno api", repository.remote.exercisesImageApi(queries).toString())
            try {
                val response = repository.remote.exercisesImageApi(queries)

                imageResponse.value = handleExercisesResponse(response)
            } catch (e: Exception){
                imageResponse.value = NetworkResult.Error(e.toString())
            }
    ...
    private fun handleExercisesResponse(response: Response<String>): NetworkResult<String>? {
        when {
